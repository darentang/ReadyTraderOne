...
bidask = pd.read_csv("best_ask_bid.csv")
bidask["Time"] += df.query("Competitor == 'Tradies' and Operation == 'Insert'")["Time"].values[0]
...
bidask = bidask.query(f"Time > {time_range[0]} and Time < {time_range[1]}")
...
ax.plot(bidask["Time"], bidask["bid"], color='r')
ax.plot(bidask["Time"], bidask["ask"], color='b')
