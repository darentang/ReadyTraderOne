...
class AutoTrader(BaseAutoTrader):
    ...
    def inventory(self, side):
        ...
        if self.etf_position > 0 and self.clamp > self.future_orderbook.midpoint() and self.ask_id == 0 and self.time_out(Side.SELL):
            q = self.etf_ub
            return np.min((40, self.constants.MAX_VOLUME - q))

        if self.etf_position < 0 and self.future_orderbook.midpoint() > self.clamp and self.bid_id == 0 and self.time_out(Side.BUY):
            q = self.etf_lb
            return np.min((40, q + self.constants.MAX_VOLUME))
    ...
    def pricing(self, side):
        fee_fraction = 0.02e-2
        # +100
        if self.etf_orderbook.best_bid() * (1 - fee_fraction) >= self.future_orderbook.midpoint() + 100 and side == Side.SELL:
            # Pricing
            self.logger.info("Free money ask")
            self.cancel(self.bid_id)
            return self.etf_orderbook.best_bid()

        elif self.etf_position > 0 and self.clamp >= self.future_orderbook.midpoint() and self.ask_id == 0 and self.time_out(Side.SELL):
            if self.future_orderbook.best_ask() < self.etf_orderbook.best_ask():
                price = self.future_orderbook.best_ask()
            else:
                price = self.etf_orderbook.best_ask()
            self.logger.info("Ask Clamp, Price %d", price)
            return price


        if self.etf_orderbook.best_ask() * (1 + fee_fraction) <= self.future_orderbook.midpoint() - 100 and side == Side.BUY:
            # Pricing
            self.logger.info("Free money bid")
            self.cancel(self.ask_id)
            return self.etf_orderbook.best_ask()

        elif self.etf_position < 0 and self.future_orderbook.midpoint() >= self.clamp  and self.bid_id == 0 and self.time_out(Side.BUY):
            # self.logger.info("%s", "buy clamp")
            if self.future_orderbook.best_bid() > self.etf_orderbook.best_bid():
                price = self.future_orderbook.best_bid()
            else:
                price = self.etf_orderbook.best_bid()
            self.logger.info("Bid Clamp, Price %d", price)
            return price


        discount = - np.sign(self.etf_position) * np.floor( np.abs(self.etf_position) * 3 / 100 ) * 100
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        # Delta and Clamp
        delta = 0.002*self.future_orderbook.midpoint()
        delta -= delta % 1.0
        min_price = self.future_orderbook.midpoint() - delta
        max_price = self.future_orderbook.midpoint() + delta
        if self.etf_orderbook.midpoint() < min_price:
            self.clamp = min_price
        elif self.etf_orderbook.midpoint() > max_price:
            self.clamp = max_price
        else:
            self.clamp = self.etf_orderbook.midpoint()
