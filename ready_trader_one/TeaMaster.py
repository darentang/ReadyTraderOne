...
class Orderbook:
    ...
    def midpoint(self):
        if self.total_volume != 0:
            return int(0.5 * (self.best_ask() + self.best_bid()) // 100) * 100
...
class AutoTrader(BaseAutoTrader):
    ...
    def inventory(self, side):
        if side == Side.BUY:
            q = self.etf_position
            return int(np.min(( -q + 100, self.constants.MAX_ORDER)))
        else:
            q = self.etf_position
    ...
    def insert(self, side, volume, price, lifespan=Lifespan.GOOD_FOR_DAY):
        ...
        if len(self.command_buffer) + 1 < self.constants.MAX_MESSAGE:
            ...
            self.logger.info("Not inserted due to frequency limit")
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if self.etf_orderbook.best_bid() * (1 - fee_proportion) >= self.future_orderbook.midpoint() + 100 and sell > 0 and self.ask_id == 0 and self.time_out(Side.SELL):
            ...
            # Pricing
            # price = f*self.future_orderbook.best_ask() + (1 - f)*self.etf_orderbook.best_ask()

            price = self.etf_orderbook.best_bid()
            self.insert(Side.SELL, sell, price, Lifespan.FILL_AND_KILL)
        ...
        if self.etf_orderbook.best_ask() * (1 + fee_proportion) <= self.future_orderbook.midpoint() and buy > 0 and self.bid_id == 0 and self.time_out(Side.BUY):
            ...
            # Pricing
            # price = f*self.future_orderbook.best_bid() + (1 - f)*self.etf_orderbook.best_bid()

            price = self.etf_orderbook.best_ask()
            # price = self.etf_orderbook.midpoint()
            self.insert(Side.BUY, buy, price, Lifespan.FILL_AND_KILL)
