...
class AutoTrader(BaseAutoTrader):
    ...
    def quote(self):
        ...
        self.bid_price -= self.bid_price % 100
        self.ask_price -= self.ask_price % 100 
        
        self.bid_id = next(self.order_ids)
        self.ask_id = next(self.order_ids)
        if self.bid_volume > 0 and self.bid_price > 0:
            self.send_insert_order(self.bid_id, Side.BUY, int(self.bid_price), int(self.bid_volume), Lifespan.GOOD_FOR_DAY)
        if self.ask_volume > 0 and self.ask_price > 0:
            self.send_insert_order(self.ask_id, Side.SELL, int(self.ask_price), int(self.ask_volume), Lifespan.GOOD_FOR_DAY)
        self.quote_time = self.get_time()
        self.logger.info("Placing quotes (%d, %d). Bid: %d @ $%d, Ask: %d @ $%d", self.bid_id, self.ask_id, self.bid_volume, self.bid_price / 100, self.ask_volume, self.ask_price / 100)
